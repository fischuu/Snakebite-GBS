...
if(!is.element("snakemake",ls())){
  projFolder <- "/hdd/data1/Projects/GBS/Example/"
  pipelineFolder <- "/home/fischuu/git/Pipeline-GBS/"
  refGenome.file <- ""
}
...
knitr::include_graphics(file.path(pipelineFolder,"workflow.png"))
```
...
out <- data.frame(c("Number of raw-samples",
                    ...
                    "Used reference genome reference"),
                  c(nrow(rawsamples),
                    nrow(barcodes),
                    ncol(sampleInfo)-1,
...
knitr::kable(out, col.names = NULL) %>% kable_styling
```
...
knitr::kable(out, col.names=FALSE) %>% kable_styling
out <- as.data.frame(as.matrix((summary(tmp[,5]))))
colnames(out) <- "5-point summary of total sequences"
knitr::kable(t(out)) %>% kable_styling
```
...
par(mar=c(10,5,1,1))
plotFastQCFeature(conFastQC.R1, conFastQC.R2, feature="total_deduplicated_percentage")
```
...
par(mar=c(10,5,1,1))
plotFastQCFeature(conFastQC.R1, conFastQC.R2, feature="X.GC")
```
...
par(mar=c(10,5,1,1))
plotFastQCFeature(conFastQC.R1, conFastQC.R2, feature="avg_sequence_length")
```
...
par(mar=c(10,5,1,1))
plotFastQCFeature(trimmedFastQC.R1, trimmedFastQC.R2, feature="total_deduplicated_percentage")
```
...
par(mar=c(10,5,1,1))
plotFastQCFeature(trimmedFastQC.R1, trimmedFastQC.R2, feature="X.GC")
```
...
par(mar=c(10,5,1,1))
plotFastQCFeature(trimmedFastQC.R1, trimmedFastQC.R2, feature="avg_sequence_length")
```
...
names(tmp) <- c("Avg. total sequences",
                "Avg. poor quality",
                "Avg. GC Percent",
                "Avg. deplication percent",
                "Avg. sequence length")
knitr::kable(apply(tmp,2,mean))  %>% kable_styling
out <- as.data.frame(as.matrix((summary(tmp[,5]))))
colnames(out) <- "5-point summary of total sequences"
knitr::kable(t(out)) %>% kable_styling
```
...
if(refAvail){
  refGenome <- importFA(file.path(projFolder, "references", refGenome.file))
  out <- summary(refGenome)
  out #knitr::kable(t(out)) %>% kable_styling
}
```
...
mockReference <- importFA(file.path(projFolder,"FASTQ", "TRIMMED", mockReference.file))
mockClusters <- importFA(file.path(projFolder,"FASTQ", "TRIMMED", mockClusters.file))
...
```{r mock build stats}
summary(vsearchIN)
#rm(c("vsearchIN", "vsearchOUT")
```

Basic information on the identified clusters, used as mock reference

```{r summarise mock reference clusters}
summary(mockClusters)
```

Total length of the input fasta to build reference: `r format(sum(nchar(vsearchIN)), big.mark=",")` bp.

Total length of the Mock Reference Clusters: `r format(sum(nchar(mockClusters)), big.mark=",")` bp.

Total length of the Mock Reference (Used as reference): `r format(sum(nchar(mockReference)), big.mark=",")` bp.

## Theoretical coverage of Mock per sample
Given the length of the mock reference assembly and the total number of sequenced bases per sample, we can estimate the theoretical coverage of the mok for each sample

Currently I hard-code here the paired-end reads and assume equal lengths and reads in both pairs. Adjust this later 'to reality'...

```{r mock coverage}
mockLength <- sum(nchar(mockReference))
mockCov <- 2*mqcStats.trim[,10] * mqcStats.trim[,5]/mockLength
names(mockCov) <- mqcStats.trim[,1]
barplot(mockCov, ylab="'x'-fold Coverage")
...
```{r pear settings}
cat(paste(pearIn[(startPos[1]+2):(startPos[1]+12)],"\n"))
```

```{r report Pear stats}
cat("Number of used samples for the mock reference:", length(startPos),"\n")
cat(paste(pearIn[c(startPos, startPos+1)],"\n", sep=""))
...
barplot(t(atDist), main="A,C,T,G distribution")
```

### Assembling statistics
Statistics on how many reads have been assembled (=had a minimum overlap of 10 bases between R1 and R2), have been discarded ( due to length and/or quality) or were not possible to overlap.
...
knitr::kable(stitching)
```

### Discarded reads
WARNING!!! THIS CURRENTLY WORKS ONLY, IF ONLY ONE SAMPLE WAS USED TO BUILD THE MOCK!!!
...
summary(discardedReads)
```

Quality values for the first 100.000 sequences
```{r quality measues}
knitr::kable(table(unlist(strsplit(discardedReads$qual[1:100000],""))))
```
...
* 0: Read aligned aginst forward strand
...
```{r sam.flagstats}
if(refAvail){
samFlagstats <- read.table(file.path(projFolder,"SAM","mockToRef.samflags"))
totalLoci <- samFlagstats$V1[samFlagstats$V2==0] + samFlagstats$V1[samFlagstats$V2==16]
knitr::kable(samFlagstats)
} else {
  totalLoci <- 0
}
```
...
Please note, if the difference between these two numbers is too large, it indicates that the mock reference needs to be stricted merged.

In total `r totalBases` bases are covered in the reference genome from mock cluster sequences.
...
barplot(mappedLoci[,2], main="Alignments per Chromosome in Reference")
}
```

```{r SAMbed}
if(refAvail){
 SAMLoci <- read.table(file.path(projFolder,"SAM","mockToRef.bed"))
}
```
...
```{r}
if(refAvail){
barplot(avgDist, main="Avg cluster mapping distance per Chromosome")
}
```

```{r}
if(refAvail){
flankingCounts <- read.table(file.path(projFolder,"SAM","mockToRef.combined.fa.counts"))
} else {
  flankingCounts <- 0
}
```
...
p <- barplot(mapStats, col=c(viridis(20)[8], viridis(20)[16]))
...
barplot(mapStats[1,] / (apply(mapStats,2,sum)), ylim=c(0,1), ylab="Mapping in Percent")
```

## Mock Reference coverage
...
```{r lorenz}
plot(cumsum(sort(clusterCoverage[,2] / sum(clusterCoverage[,2]))), type="l")
...
barplot(table(coveredClusters), xlab="Samples with reads on cluster")
```
...
barplot(nonHittedClusters/nrow(clusterCoverage), ylim=c(0,1))
...
```{r vizCusterCoverage}
plot(smooth.spline(clusterCoverage[,1], log(clusterCoverage[,2]+1), all.knots=FALSE), type="l", xlab="Cluster", ylab="Log-coverage")
...
plot(smooth.spline(clusterCoverage[,1], log(clusterCoverage.std[,2]+1), all.knots=FALSE), type="l", xlab="Cluster", ylab="")
...
```{r}
pairs(vcf.PCA$x[,1:10])

if(ncol(sampleInfo)>1){
for(i in 2:ncol(sampleInfo)){
  pairs(vcf.PCA$x[,1:10], col=as.numeric(as.factor(sampleInfo[,i])), pch=20)
}
}
```
...
### t-SNE
```{r}
TSNEout <- tsne(vcf.PCA$x[,1:10], k=5)
...
```{r}
# Kernel PCA
  KPCAres <- kpca(vcf.PCA$x[,1:10])

if(ncol(sampleInfo)>1){
  for(i in 2:ncol(sampleInfo)){
     pairs(rotated(KPCAres)[,1:10], col=as.numeric(as.factor(sampleInfo[,i])), pch=20)
  }
...
```{r}
# LLE
  lleres <- lle(vcf.PCA$x[,1:10], m=7, k=17, v=0.99)
...
configFile <- readLines(file.path(pipelineFolder, "Genotype-pipeline_config-racoon.yaml"))
```

```{r print configuration}
knitr::kable(configFile)
```
...
```{r import conda file}
condaFile <- readLines(file.path(pipelineFolder, "Genotype.yaml"))
```

```{r print conda}
knitr::kable(condaFile)
```
