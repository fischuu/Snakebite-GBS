...
if(!is.element("snakemake",ls())){
  projFolder <- "~/projectsTmp/Example"
  pipelineFolder <- "~/git/Pipeline-GBS/"
  pipelineConfig <- "~/git/Pipeline-GBS/GBS-pipeline_config-example.yaml"
  refGenome.file <- "/scratch/project_2001746/Example/references/Vulpes_vulpes.VulVul2.2.dna.toplevel.fa"
}
...
```{r quality measues}
out_html <- knitr::kable(table(unlist(strsplit(discardedReads$qual[1:100000],""))), "html")
kable_styling(out_html, "striped", position = "left")
```
...
barplot(mappedLoci[,2], main="Alignments per Chromosome in Reference", col=viridis(20)[8])
}
```
...
```{r}
if(refAvail){
barplot(avgDist, main="Avg cluster mapping distance per Chromosome", col=viridis(20)[8])
}
```
...
barplot(table(coveredClusters), xlab="Samples with reads on cluster", col=viridis(20)[8])
```

Now we have here the number of reads per coverage class. That means, instead of having it binary as in the previous plot, wenow count all the reads per coverage group.
...
if(sum(is.element(colnames(sampleInfo), "casecontrol"))>1){
  mmCase <- 2
  mmControl <- 5
