...
if(!is.element("snakemake",ls())){
  ...
  pipelineConfig.file <- "/scratch/project_2001746/BSF/GBS-pipeline_config-BSF.yaml"
...
```{r}
qcFiles.raw <- list.files(file.path(projFolder,"QC", "RAW"), pattern="*qualdist.txt")
qcQual.raw <- fread(file.path(projFolder,"QC", "RAW", qcFiles.raw[1]), skip=1)
colnames(qcQual.raw) <- c(gsub("_qualdist.txt","", qcFiles.raw[1]), "Quality")

for(i in 2:length(qcFiles.raw)){
    tmp <- fread(file.path(projFolder,"QC", "RAW", qcFiles.raw[i]), skip=1)
    colnames(tmp) <- c(gsub("_qualdist.txt","", qcFiles.raw[i]), "Quality")
    qcQual.raw <- merge(qcQual.raw, tmp, all=TRUE)
}
qcQual.raw[is.na(qcQual.raw)] <- 0

```

```{r}
plotThis <- as.matrix(qcQual.raw[,-1])
rownames(plotThis) <- as.vector(as.matrix(qcQual.raw[,1]))
...
cutLogs <- fread(file.path(projFolder, "logs", "CUTADAPT", cutLogs.files[1]), skip="Overview of removed sequences")
cutLogs <- cutLogs[,1:2]
colnames(cutLogs) <- c("length", gsub(".log", "",cutLogs.files[1]))

for(i in 2:length(cutLogs.files)){
  tmp <- fread(file.path(projFolder, "logs", "CUTADAPT", cutLogs.files[i]), skip="Overview of removed sequences")
  tmp <- tmp[,1:2]
...
```{r}
barplot(table(string.merge$V1), las=2)
```
...
```{r}
hist(as.vector(as.matrix(vcf$map[,4])), main="SNP loci distribution", xlab="Position")
```
...
barplot(missingnessPerSample, las=2, main="Missing variants per sample")
...
```{r}
barplot(apply(vcf$genotypesInfo==".,.",2,sum)/noVariants*100, las=2, main="Missing variants per sample (in %)")
```
...
```{r}
if(refAvail){
  barplot(apply(vcfRef$genotypesInfo==".,.",2,sum)/noVariants.ref*100, las=2, main="Missing variants per sample (in %)")
}
```
