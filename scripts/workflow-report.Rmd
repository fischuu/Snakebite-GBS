...
if(!is.element("snakemake",ls())){
  projFolder <- "/scratch/project_2001746/Whitefish_benchMS6"
...
```{r}
tmp <- cumsum(sort(apply(DP,2,sum)))
plot(tmp, type="l")
...
dataForBarplot <- apply(plotThis,1,table)
df <- data.frame(matrix(unlist(dataForBarplot), nrow=length(dataForBarplot), byrow=TRUE))
rownames(df) <- rownames(vcf$genotypes)
barplot(df, col=c("green", "blue", "red", "black"), las=2)
```
