...
```{r reads per final coverage group}
readsPerCoverageGroup <- c()
...
```{r reads per final coverage group in percent}
barplot(readsPerCoverageGroup/sum(readsPerCoverageGroup)*100, xlab="Coverage group", ylab="Reads on cluster group (in %)", ylim=c(0,100), col=viridis(20)[8])
```
...
vcf <- importVCF(file.path(projFolder, "VCF", "FinalSetVariants_finalMock.vcf"))
if(refAvail){
...
```{r}
hist(as.matrix(vcf$map[,4]), xlab="SNP pos in bp on Cluster")
```
