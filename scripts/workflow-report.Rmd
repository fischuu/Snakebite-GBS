...
```{r}
cutLogs.files <- list.files(file.path(projFolder, "logs"), pattern="cutadapt*")
cutLogs <- fread(file.path(projFolder, "logs", cutLogs.files[1]), skip="Overview of removed sequences")
cutLogs <- cutLogs[,1:2]
colnames(cutLogs) <- c("length", gsub(".log", "",cutLogs.files[1]))

for(i in 2:length(cutLogs.files)){
  tmp <- fread(file.path(projFolder, "logs", cutLogs.files[i]), skip="Overview of removed sequences")
  tmp <- tmp[,1:2]
...
Distribution of identifed contigs across the reference genome

```{r, eval=FALSE}
  tmp <- summary(abs(string.merge$V5 - string.merge$V4))
  out <- data.frame(names(tmp), tmp)
  out_html <- knitr::kable(out, col.names = NULL, "html")
  kable_styling(out_html, "striped", position = "left")
```
