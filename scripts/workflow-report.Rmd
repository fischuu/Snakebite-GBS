...
```{r sam.flagstats}
if(refAvail){
samFlagstats <- read.table(file.path(projFolder,"BAM", "Mockref", "mockToRef.sam.flagstat"))
...
```{r}
if(refAvail){
flankingCounts <- read.table(file.path(projFolder,"BAM","Mockref","mockToRef.combined.fa.counts"))
} else {
  flankingCounts <- 0
}
```
