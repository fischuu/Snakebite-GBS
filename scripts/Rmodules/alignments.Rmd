...
tmp[order(totCov, decreasing = TRUE)[1:50],]
```
...
For that we try now to what percent we find which combination in the samples. First the top ten 
...
hitsLeft4 <- list()
for(i in 1:length(flankingLeft)){
  hitsLeft1[[i]] <- as.vector(adist(enz1, flankingLeft[[i]]))
  hitsLeft2[[i]] <- as.vector(adist(enz2, flankingLeft[[i]]))
  hitsLeft3[[i]] <- as.vector(adist(revcomp(enz1), flankingLeft[[i]]))
  hitsLeft4[[i]] <- as.vector(adist(revcomp(enz2), flankingLeft[[i]]))
}
...
for(i in 1:length(flankingLeft)){
  hitsRight1[[i]] <- as.vector(adist(enz1, flankingRight[[i]]))
  hitsRight2[[i]] <- as.vector(adist(enz2, flankingRight[[i]]))
  hitsRight3[[i]] <- as.vector(adist(revcomp(enz1), flankingRight[[i]]))
  hitsRight4[[i]] <- as.vector(adist(revcomp(enz2), flankingRight[[i]]))
}
```
...
* Check, how those loci from above table 1.50 relate to the in-situ processed digestion. Further, check how many reads align to "real spots" compared to non-wished ones.
