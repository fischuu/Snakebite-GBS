...
Import the mapping statistics to the preliminary Mock reference
...
These are the amounts of clusters with reads from different samples. For example, 4 on the x-axis gives the amount of clusters with reads from four different samples. Naturally, one would like to see here as high as possible bars on the right hand side of the plot.
...
Now we check still the precise coverage per sample for the sake of simplicity I do it here for the first sample only and also only for the first at most 50 chromosome. Here, one can possibly see issues in the reference genome, when the coverage is not uniformly. Areas with a coverage of more than 20 reads are indicated in red, those below 10 are in blue and areas in between are represented with black.
...
And then the sme distribution, but still weighted by the reads, so here we take each read into account, to determine the length distribution (this is the distribtion, you should look at when you check the QC of the sequencing run)
...
```{r}
tmp <- fc.quant$expValues[,-1]
totCov <- apply(tmp,1,sum)
...
We consider the flanking sites of the aligned reads.
...
This table is as we take it from the data:
