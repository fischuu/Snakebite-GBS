...
for(i in 2:length(gtf.files)){
  number_loci <- sum(substr(readLines(file.path(projFolder, "Stringtie", gtf.files[i]), n=10),1,1)!="#")>0
  if(number_loci>0) gtfs[[i]] <- importGTF(file.path(projFolder, "Stringtie", gtf.files[i]), level="transcript")
  
  samples_without_loci <- c(samples_without_loci, gtf.names[i])
  if(number_loci>0) remove_those <- c(remove_those, i)
}
...
In case there were samples without any identified alignment locus on the reference genome, they are printed here:

```{r}

```

Visualisation, how many alignments loci do the different samples have along the reference genome

```{r, fig.width=10}
par(mar=c(7,5,0,0))
barplot(sapply(gtfs, nrow), las=2)
```
...
Basic visualisation, binary, if a sample has reads on a locus we plot it dark red, otherwise light yellow. Ideally, this is now just a red block.

```{r, fig.width=10}
image(fc.quant$expValues>0)
```
