...
### Alignment rate (full in-silico)

First we check how well our reads align to complete in-silico prediction. As this is not size-filtered, please keep in mind that this represents more or less the complete reference genome and hence
the alignment rate between the reference genome and the full in-silico prediction should not differ much.

```{r}
fullAR.files <- list.files(file.path(projFolder, "BAM", "Insilico", "full"), pattern="*.flagstat")

fullAR.tmp <- readLines(file.path(projFolder, "BAM", "Insilico", "full", fullAR.files[1]))[5]
fullAR <- as.numeric(strsplit(strsplit(fullAR.tmp, "\\(")[[1]][2], "\\%")[[1]][1])

for(i in 2:length(fullAR.files)){
fullAR.tmp <- readLines(file.path(projFolder, "BAM", "Insilico", "full", fullAR.files[i]))[5]
fullAR[i] <- as.numeric(strsplit(strsplit(fullAR.tmp, "\\(")[[1]][2], "\\%")[[1]][1])
}

names(fullAR) <- gsub(".sam.flagstat", "", fullAR.files)

```

```{r}
par(mar=c(7,3,1,1))
barplot(fullAR, col=report.colours[1], las=2, ylim=c(0,100))
```

### Alignment rate (size-selected in-silico)
And now, we align the reads against the fragments that we should have, given that our restriction and size selection worked.

```{r}
selectedAR.files <- list.files(file.path(projFolder, "BAM", "Insilico", "selected"), pattern="*.flagstat")

selectedAR.tmp <- readLines(file.path(projFolder, "BAM", "Insilico", "selected", selectedAR.files[1]))[5]
selectedAR <- as.numeric(strsplit(strsplit(selectedAR.tmp, "\\(")[[1]][2], "\\%")[[1]][1])

for(i in 2:length(selectedAR.files)){
selectedAR.tmp <- readLines(file.path(projFolder, "BAM", "Insilico", "selected", selectedAR.files[i]))[5]
selectedAR[i] <- as.numeric(strsplit(strsplit(selectedAR.tmp, "\\(")[[1]][2], "\\%")[[1]][1])
}

names(selectedAR) <- gsub(".sam.flagstat", "", selectedAR.files)

```

```{r}
par(mar=c(7,3,1,1))
barplot(rbind(selectedAR, fullAR-selectedAR), col=report.colours[1:2], las=2, ylim=c(0,100))
legend("bottomright", pch=c(20,20), col=c(report.colours[2], report.colours[1]), legend=c("Full model", "Size selected"), fill="white")

```

### Coverage (full in-silico prediction)

The pipeline currently takes only paired-end reads and we use here the idxstats output of samtools. In case of paired-end reads, we get because
of that here roughly twice the number of aligned reads (one for the forward and one for the reverse strand read, in cause reads were properly
paired). For the sake of simplicity, we take here half of that measure, knowing that this is not 100% accurate, but well enough estiamte for the
real value. In a future version (e.g. when single-end support will be added), we should improve that step.

```{r}
fullC.files <- list.files(file.path(projFolder, "BAM", "Insilico", "full"), pattern="*.coverage")
fullC <- read.table(file.path(projFolder, "BAM", "Insilico", "full", fullC.files[1]))
colnames(fullC) <- c("Location", gsub(".coverage", "", fullC.files[1]))
fullC[,2] <- ceiling(fullC[,2]/2)

for(i in 2:length(fullC.files)){
  fullC.tmp <- read.table(file.path(projFolder, "BAM", "Insilico", "full", fullC.files[i]))
  fullC.tmp[,2] <- ceiling(fullC.tmp[,2]/2)
  colnames(fullC.tmp) <- c("Location", gsub(".coverage", "", fullC.files[i]))

  fullC <- merge(fullC, fullC.tmp, by="Location")
}

fullC <- fullC[-which(fullC$Location=="*"),]
```

### Coverage (size-selected in-silico)
...
```{r}
selectedC.files <- list.files(file.path(projFolder, "BAM", "Insilico", "selected"), pattern="*.coverage")
selectedC <- read.table(file.path(projFolder, "BAM", "Insilico", "selected", selectedC.files[1]))
colnames(selectedC) <- c("Location", gsub(".coverage", "", selectedC.files[1]))
selectedC[,2] <- ceiling(selectedC[,2]/2)

for(i in 2:length(selectedC.files)){
  selectedC.tmp <- read.table(file.path(projFolder, "BAM", "Insilico", "selected", selectedC.files[i]))
  selectedC.tmp[,2] <- ceiling(selectedC.tmp[,2]/2)
  colnames(selectedC.tmp) <- c("Location", gsub(".coverage", "", selectedC.files[i]))

  selectedC <- merge(selectedC, selectedC.tmp, by="Location")
}

selectedC <- selectedC[-which(selectedC$Location=="*"),]
```

### Fragment length distribution (full in-silico prediction)

```{r}
fullSizeC <- merge(insilico.full.length, fullC, by="Location")

lengths <- names(table(fullSizeC$Length))

lengths <- sort(as.numeric(lengths))[1:2000]


fullSizeDist <- matrix(0, ncol=ncol(fullC) - 1 , nrow=length(lengths))

for(i in 1:length(lengths)){
  tmpRows <- which(fullSizeC$Length==lengths[i])
  fullSizeDist[i,] <- apply(fullSizeC[tmpRows,-c(1:2)],2,sum)
}

```

```{r}
plot(lengths,fullSizeDist[,1], type="l", ylim=c(0, max(fullSizeDist)), xlab="Fragment length", ylab="Sum read coverage")
for(i in 2:ncol(fullSizeDist)){
  lines(lengths,fullSizeDist[,i])
}
...
```{r}
plot(lengths,log(fullSizeDist[,1]), type="l", ylim=c(0, max(log(fullSizeDist))), xlab="Fragment length", ylab="log(sum read coverage)")

for(i in 2:ncol(fullSizeDist)){
  lines(lengths,log(fullSizeDist[,i]))
}
```

### Fragment length distribution (size-selected in-silico prediction)

```{r}
selectedSizeC <- merge(insilico.size.length, selectedC, by="Location")

lengths <- names(table(selectedSizeC$Length))

selectedSizeDist <- matrix(0, ncol=ncol(selectedC) - 1 , nrow=length(lengths))

for(i in 1:length(lengths)){
  tmpRows <- which(selectedSizeC$Length==lengths[i])
  selectedSizeDist[i,] <- apply(selectedSizeC[tmpRows,-c(1:2)],2,sum)
}

```

```{r}
plot(lengths,selectedSizeDist[,1], type="l", ylim=c(0, max(selectedSizeDist)), xlab="Fragment length", ylab="Sum read coverage")
for(i in 2:ncol(selectedSizeDist)){
  lines(lengths,selectedSizeDist[,i])
}
```

```{r}
plot(lengths,log(selectedSizeDist[,1]), type="l", ylim=c(0, max(log(selectedSizeDist))), xlab="Fragment length", ylab="log(sum read coverage)")

for(i in 2:ncol(selectedSizeDist)){
  lines(lengths,log(selectedSizeDist[,i]))
}
```

### Location changes between full and size-selected
We compare how the alignment numbers change between the full in-silico and size-selected in-silico predictions change. I restrict the full model to the same locations present in the size-selected reference and then see, how the coverage changes.

Positive values indicate moves from the selected to full prediction model, negative values from full model to selected model. If values are large, that might indicate large sequence similarities between different loci, outside the aimed size selection.

```{r}
fullSelectedC <- merge(fullC, selectedC, by="Location")

fullSelected.changes <- fullSelectedC[,2:ncol(fullC)] - fullSelectedC[,(ncol(fullC)+1):ncol(fullSelectedC)]

nSamples <- ncol(fullC)-1
fullSelected.up <- c()
fullSelected.down <- c()

for(i in 1:nSamples){
 fullSelected.up[i] <- sum(fullSelected.changes[fullSelected.changes[,i]>0,i])  
 fullSelected.down[i] <- sum(fullSelected.changes[fullSelected.changes[,i]<0,i])
}
```

```{r}
barplot(fullSelected.down, col=report.colours[1], ylim=range(c(fullSelected.down, fullSelected.up)))
barplot(fullSelected.up, col=report.colours[2], add = TRUE)
```
